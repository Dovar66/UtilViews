package com.dovar.utilviews.utils;
...
public class TimeUtils {
    ...
    public static String getPublishTime(long time, SimpleDateFormat format) {
        Date updateTime = new Date(time);
        Date nowTime = new Date(System.currentTimeMillis());
        //天
        if ((nowTime.getTime() - updateTime.getTime()) / (24 * 60 * 60 * 1000) > 0) {
            return milliseconds2String(time,format );
//            return (nowTime.getTime()-updateTime.getTime())/(24*60*60*1000)+"天前";
        } else {
            //小时
            if ((nowTime.getTime() - updateTime.getTime()) / (60 * 60 * 1000) > 0) {
                return (nowTime.getTime() - updateTime.getTime()) / (60 * 60 * 1000) + "小时前";
            } else {
                //分钟
                if ((nowTime.getTime() - updateTime.getTime()) / (60 * 1000) > 0) {
                    return (nowTime.getTime() - updateTime.getTime()) / (60 * 1000) + "分钟前";
                } else {
                    return "1分钟前";
                }
            }
